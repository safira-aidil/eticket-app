<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            @if(session('success'))
                <div class="mb-4 p-4 bg-green-500 text-white rounded-2xl shadow-lg font-bold text-sm animate-pulse">
                    {{ session('success') }}
                </div>
            @endif
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-gray-50">
                        ...
                        <tr class="hover:bg-indigo-50/30 transition">
                            <td class="px-6 py-5">
                                <p class="text-[10px] text-indigo-500 font-bold mb-1">#{{ $ticket->ticket_number }} | {{ $ticket->user->name ?? 'User' }}</p>
                                <p class="font-bold text-gray-800">{{ $ticket->title }}</p>
                                <p class="text-xs text-gray-400 truncate w-64">{{ $ticket->description }}</p>
                            </td>
                            ...
                            <td class="px-6 py-5">
                                <form action="{{ route('admin.tickets.updateStatus', $ticket) }}" method="POST" class="flex items-center justify-center space-x-2">
                                    @csrf
                                    @method('PATCH')
                                    ...
                                    <button type="submit" class="p-2 bg-indigo-700 text-white rounded-lg hover:bg-indigo-800 transition shadow-md">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                                    </button>
                                </form>
                            </td>
                            <td class="px-6 py-5 text-right text-[11px] font-black text-gray-500">
